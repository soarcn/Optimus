...
abstract class MockResponseSupplier {
    ...
    internal fun get(cls: KClass<out MockResponse>, method: Method): Response<*> {
        val value = load(method.kotlinFunction.toString())
    ...
    internal fun index(kFunction: KFunction<*>, cls: KClass<out MockResponse>): Int {
        val value = load(kFunction.toString())
        val field = cls.declaredMemberProperties.firstOrNull { it.name == value }
            ?: findDefaultField(cls)
    ...
    companion object {
        ...
        fun memory(map: HashMap<String, String> = HashMap()):MockResponseSupplier {
            return InMemoryMockResponseSupplier(map)
        }
    }
}
